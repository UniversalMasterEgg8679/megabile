...
- (void) startProgramm {
    ...
    NSLog(@"ROOT %@",[ApplicationData getRootCategory]);
    vcCatalog.currentCategory.parentId =[ApplicationData getRootCategory]; // SET PARENT = ROOT CATEGORY
...
-(void) checkAPIandDownloadCatalog
{
    ...
    self.apiUpdateDate = [[jsonDict objectForKey:@"updatedate"] intValue];
    self.apiProducts = [[jsonDict objectForKey:@"products"] intValue];
    NSLog(@"API: PRODUCTS: %i UDATEDATE: %i",self.apiProducts,self.apiUpdateDate);
    
    // CHECK FOR AN ERROR AND SHOW ERROR IF EXISTS
    NSString *errorAPI = [jsonDict objectForKey:@"error"];
    NSLog(@"API: ERROR: %@",errorAPI);
    ...
    if ([errorAPI length] > 0) {
        ...
        // SERVER UPTIME
        if (self.apiUpdateDate > 0) {
            if ( [self isDatesEquals:self.apiUpdateDate] == NO ) {
...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response
{
    ...
    NSLog(@"API: didReceiveResponse %.2f Mb",totalMb);
}
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
    ...
    // WRITE NEW DATE
    [self saveUpdateDate:self.apiUpdateDate];
...
-(void)parser:(NSXMLParser *)parser foundCharacters:(NSString *)string
{
    ...
    if ([self.currentNode isEqualToString:API_KEY] ) {
        self.apiKey = string;
    }
}
...
-(BOOL)isDatesEquals:(int) date {
    ...
    NSLog(@"API: savedDate:%i date:%i",savedDate,date);
    BOOL result = NO;
...
-(void)saveUpdateDate:(int) date {
    NSLog(@"API: New savedDate:%i",date);
